...
Beispiel:
  ...
  printf '%s\n' 'Korrigiere nur Rechtschreibung; ändere keine Aussagen.' \
    > .local-agent/task.md
  tools/local-agent.sh .local-agent/task.md README.md docs/product/VISION.md
...
shift
[[ "$task_file" != /* && "$task_file" != *".."* ]] || \
  fail "$EXIT_USAGE" "Die Aufgabendatei muss ein relativer Pfad ohne '..' sein."
...
for file in "$@"; do
  ...
  [[ -f "$file" && ! -L "$file" ]] || \
    fail "$EXIT_USAGE" "Erlaubte Datei fehlt, ist kein reguläres File oder ist ein Symlink: $file"
  git ls-files --error-unmatch -- "$file" >/dev/null 2>&1 || \
    fail "$EXIT_USAGE" "Datei ist nicht versioniert: $file"
  duplicate=false
  if (( ${#allowed_files[@]} > 0 )); then
    for allowed_file in "${allowed_files[@]}"; do
      [[ "$allowed_file" == "$file" ]] && duplicate=true
    done
  fi
